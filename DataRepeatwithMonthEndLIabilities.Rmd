...
p_load(haven, tidyverse, ggplot2,sjlabelled, lubridate, 
  readxl, xtable, gapminder, kable, kableExtra, RColorBrewer,zoo)
p_load(astsa)
my_folder <- here("data")
...
Dec 1999- Jan 2000 stand out as a large spike in net withdrawals at the monthly level.   
...
# So, honestly, it seems like I don't even really need the 
# Extra time series stuff. The trended one is fine
# Just look for large residuals there!
...
Note that, since there is no "return to trend" yet in NIC, the nonparametric fit sees the maintained pandemic behavior as the trend. 
...
#netwithdrawal_pred_1999_spl= datagg %>% mutate(.fitted = fit1$fitted.values) %>% filter(year == 2019)    
...
gen_plots <- function(my_model, my_name) {
  ...
  names(my_model$coefficients) = c("Intercept", month_names, "year", "(year)^2")
  plot1 = jtools::plot_summs(my_model, omit.coefs = 
      c("(Intercept)", "year","(year)^2" ), ci_level = .95) + 
    ggtitle("Weekly Fixed Effects")
  plot1
